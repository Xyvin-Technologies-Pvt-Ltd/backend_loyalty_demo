...
async function startServer() {
    try {
        ...
        const server = app.listen(PORT, () => {
            ...
            const env_message = clc.yellowBright(`✓ Environment: ${NODE_ENV || 'development'}`);
            const status_message = clc.greenBright('✓ Server is up and running smoothly 🚀');

            logger.info(`Server started on port ${PORT} in ${NODE_ENV || 'development'} mode`);
