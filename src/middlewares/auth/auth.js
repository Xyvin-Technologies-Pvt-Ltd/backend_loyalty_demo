    ...
    return async (req, res, next) => {
        try {
            ...
            return protect(req, res, async () => {
                ...
                // Admin users have all permissions by default, but we still check their role if specified
                if (req.admin) {
                    ...
                    if (req.admin.role) {
                        try {
                            ...
                            // Continue execution - if we can't verify permissions, we'll default to admin access
                        }
                    }

                    logger.info(`Admin access granted to: ${req.admin._id} for restricted route`);
                    return next();
                }

                // For regular users, check if they have a role with the required permissions
                if (req.user && req.user.role) {
                    try {
                        ...
                        if (userRole && userRole.permissions) {
                            ...
                            if (hasAllPermissions) {
                                logger.info(`Permission-based access granted to user: ${req.user._id} with role: ${userRole.name}`);
                                return next();
                            }
                        }
                    } catch (error) {
                        logger.error(`Error checking user role permissions: ${error.message}`);
                    }
                }
