...
exports.createCouponCategory = async (req, res) => {
    try {
        ...
        return response_handler(res, 201, 'Coupon category created successfully', couponCategory);
    } catch (error) {
...
exports.getAllCouponCategories = async (req, res) => {
    try {
        const couponCategories = await CouponCategory.find();
        return response_handler(res, 200, 'Coupon categories retrieved successfully', couponCategories);
    } catch (error) {
        return response_handler(res, 500, 'Error retrieving coupon categories', error);
    }
};
...
exports.getCouponCategoryById = async (req, res) => {
    try {
        ...
        return response_handler(res, 200, 'Coupon category retrieved successfully', couponCategory);
    } catch (error) {
        return response_handler(res, 500, 'Error retrieving coupon category', error);
    }
};
...
exports.updateCouponCategory = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, image } = req.body;
        const couponCategory = await CouponCategory.findByIdAndUpdate(id, { title, description, image }, { new: true });
        return response_handler(res, 200, 'Coupon category updated successfully', couponCategory);
    } catch (error) {
