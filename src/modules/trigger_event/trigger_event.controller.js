...
exports.createTriggerEvent = async (req, res) => {
    try {
        ...
        return response_handler(res, 201, "Trigger event created successfully!", new_trigger_event);
...
exports.getAllTriggerEvents = async (req, res) => {
    try {
        const trigger_events = await TriggerEvent.find();
        return response_handler(res, 200, "Trigger events fetched successfully!", trigger_events);
    } catch (error) {
        return response_handler(res, 500, `Internal Server Error. ${error.message}`);
    }
}
...
exports.getTriggerEventById = async (req, res) => {
    try {
        ...
        return response_handler(res, 200, "Trigger event fetched successfully!", trigger_event);
    } catch (error) {
        return response_handler(res, 500, `Internal Server Error. ${error.message}`);
    }
}
...
exports.updateTriggerEvent = async (req, res) => {
    try {
        ...
        const trigger_event = await TriggerEvent.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!trigger_event) {
            return response_handler(res, 404, "Trigger event not found!");
        }
        return response_handler(res, 200, "Trigger event updated successfully!", trigger_event);
    } catch (error) {
        return response_handler(res, 500, `Internal Server Error. ${error.message}`);
    }
}
